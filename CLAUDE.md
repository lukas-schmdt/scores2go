...
**Auth gate** — `main.dart` → `AuthBloc` drives the root: `passwordRecovery` shows `ResetPasswordScreen`; every other state (including anonymous/unauthenticated) shows `HomeScreen` wrapped in its BLoC providers, so score browsing works without an account. `AuthState.isAuthenticated` (`lib/auth/bloc/auth_state.dart`) — not a raw `status` comparison — is the canonical "is this a signed-in session" check; it also covers `changePasswordSuccess`/`changePasswordFailed`, which leave the user signed in without re-emitting `authenticated`. `UserFavoritesBloc`/`CollectionsBloc` are keyed on `isAuthenticated` so they reset cleanly on sign-in/sign-out; `HomeBloc`/`RecentlyUsedBloc` are not user-scoped and persist across that transition. `AuthScreen` is no longer shown at the root — it's pushed as a route via `openAuthScreen`/`ensureSignedIn` (`lib/auth/screen/open_auth_screen.dart`) from anywhere a login-gated feature (Favorites, Collections, Settings account section, favoriting/collecting/feedback from Browse) needs to prompt sign-in, and pops itself once login succeeds. Backend is **Supabase** (URL and anon key are hardcoded in `main.dart`).
